...
import scipy.misc
scipy.misc.toimage(image[:,:,0],cmin=0.0,cmax=1.0).save("file.png")
# plt.plot(image.numpy()[:, :, :].astype(np.float32))
...
#安装gsutil ,利用python 调用命令行.
import subprocess
#注意下面一行的gsutil安装在了python目录总,所以一般的shell命令调用需要改成全路径调用才行.

cm='/work/share/PyEnvs/zb/bin/gsutil cp '+vocab_file +' '+data_dir
GSutil='/work/share/PyEnvs/zb/bin/gsutil'
print(cm)
import os
import os
myCmd = 'll'
